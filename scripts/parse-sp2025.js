...
// Simple parser (same as parse-exam.js)
...
// Read and parse SP 2025
const filePath = path.join(__dirname, '../HCM/HCM202 - SP 2025 - FE.txt');
const content = fs.readFileSync(filePath, 'utf8');
const questions = parseExamFile(content);

// Read existing exams.ts
const examsFilePath = path.join(__dirname, '../src/data/exams.ts');
let existingContent = '';
if (fs.existsSync(examsFilePath)) {
  existingContent = fs.readFileSync(examsFilePath, 'utf8');
}

// Extract existing exports - keep FA25
const fa25Match = existingContent.match(/export const fa25FeHalf1Questions: Question\[\] = \[[\s\S]*?\];/);
const fa25Export = fa25Match ? fa25Match[0] : '';

// Create new content with SP 2025 added
const output = `// This file is auto-generated. Do not edit manually.
import type { Question } from '@/lib/parseExam';

${fa25Export}

export const sp2025FeQuestions: Question[] = ${JSON.stringify(questions, null, 2)};
`;

fs.writeFileSync(examsFilePath, output, 'utf8');
console.log(`Parsed ${questions.length} questions from SP 2025`);
